...
export class SystemMonitor {
  ...
  private handleUncaughtError(error: Error, type: string): void {
    this.logger.fatal('SYSTEM_MONITOR', `Uncaught ${type}`, error, {
      errorType: type,
  ...
  private performHealthCheck(): void {
    try {
      ...
      if (health.status === 'critical') {
        this.logger.error('SYSTEM_MONITOR', 'Critical system health detected', undefined, {
  ...
  getSystemHealth(): SystemHealth {
    ...
    const criticalErrors = errorStats.errorsBySeverity[ErrorSeverity.CRITICAL] || 0
    if (memoryUsagePercent > 95 || successRate < 80 || criticalErrors > 0) {
      status = 'critical'
  ...
  log(level: LogLevel, category: string, message: string, details?: Record<string, unknown>): void {
    switch (level) {
      ...
      case LogLevel.WARN:
        ...
        break
      case LogLevel.ERROR:
        this.logger.error(category, message, undefined, details)
        break
      case LogLevel.FATAL:
        this.logger.fatal(category, message, undefined, details)
        break
    }
  }

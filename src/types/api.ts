...
export interface ChatManagerInterface {
  processChatRequest: (data: LLMRequestData) => Promise<LLMResponse>;
  processStreamingRequest: (data: LLMRequestData, callback: StreamingCallback) => Promise<void>;
...
export interface ConfigManagerInterface {
  ...
  saveSystemPrompt: (systemPrompt: string) => ConfigOperation;
  getTheme: () => Theme;
  saveTheme: (theme: Theme) => ConfigOperation;
  getDefaultModel: () => string;
  saveDefaultModel: (model: string) => ConfigOperation;
}

export interface MCPManagerInterface {
  listAllTools: () => Promise<MCPTool[]>;
  callTool: (serverName: string, toolName: string, args: Record<string, unknown>) => Promise<Record<string, unknown>>;
  loadFromConfig: (config: MCPConfig) => Promise<void>;
}
